...
// import { useAddbathMutation, useGetbathDatesQuery } from '@/redux/api/user';
import { format, isToday, isYesterday, parseISO, eachDayOfInterval, subDays } from 'date-fns';
import { useGetBathQuery } from '@/redux/api/user';

export default function BathPage() {
  const [bathDates, setBathDates] = useState([]);
  const [streakCount, setStreakCount] = useState(0);
  const [showCalendar, setShowCalendar] = useState(false);
  
  const { data } = useGetBathQuery();

  useEffect(() => {
    if (data?.data?.bath) {
      const dates = data.data.bath.map(date => parseISO(date));
      setBathDates(dates);
  ...
  const calculateStreak = (dates) => {
    ...
    for (let i = 0; i < sortedDates.length; i++) {
      const bathDate = new Date(sortedDates[i]);
      bathDate.setHours(0, 0, 0, 0);

      if (i === 0 && isToday(bathDate)) {
        streak++;
        currentDate = subDays(currentDate, 1);
      } else if (bathDate.getTime() === currentDate.getTime()) {
        streak++;
        currentDate = subDays(currentDate, 1);
      } else {
        break;
  ...
  const renderLast30Days = () => {
    ...
    return dateRange.map((day) => {
      ...
      const isbathed = bathDates.some(date => format(date, 'yyyy-MM-dd') === formattedDay);
      ...
      return (
        ...
        >
          ...
          <span className={`text-lg font-medium ${
            isbathed ? 'text-white' : 'text-gray-400'
          }`}>{dayNumber}</span>
          {isbathed && (
            <motion.div 
              initial={{ scale: 0 }}
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">Total bathed</p>
                <h3 className="text-2xl font-bold text-white">{bathDates.length} days</h3>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">Last bathed</p>
                <h3 className="text-2xl font-bold text-white">
                  {bathDates.length > 0 ? 
                    isToday(bathDates[0]) ? 'Today' : 
                    isYesterday(bathDates[0]) ? 'Yesterday' : 
                    format(bathDates[0], 'MMM d') : 'Never'}
                </h3>
              </div>
        ...
        {/* Recent bathing History */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.8 }}
        >
          <h2 className="text-xl font-semibold text-gray-300 mb-4">Bathing History</h2>
          {bathDates.length > 0 ? (
            <div className="space-y-3">
              <AnimatePresence>
                {bathDates.slice(0, 10).map((date, index) => (
                  <motion.div
                    key={date.getTime()}
                    initial={{ x: -20, opacity: 0 }}
                    animate={{ x: 0, opacity: 1 }}
            ...
            <div className="text-center py-12 bg-gray-800/30 rounded-xl border border-dashed border-gray-700/50">
              ...
              <h3 className="text-lg font-medium text-gray-400 mb-2">No bathing history yet</h3>
              <p className="text-gray-500 mb-4">Start tracking your bathing habits by adding your first entry</p>
            </div>
          )}
        </motion.div>
      </motion.div>
