...
// import { useAddBrushMutation, useGetdidThatDatesQuery } from '@/redux/api/user';
import { format, isToday, isYesterday, parseISO, eachDayOfInterval, subDays } from 'date-fns';
import {  useGetDidThatQuery } from '@/redux/api/user';

export default function DidThatPage() {
  const [didThatDates, setDidThatDates] = useState([]);
  const [streakCount, setStreakCount] = useState(0);
  const [showCalendar, setShowCalendar] = useState(false);
  
  const { data } = useGetDidThatQuery();

  useEffect(() => {
    if (data?.data?.didThat) {
      const dates = data.data.didThat.map(date => parseISO(date));
      setDidThatDates(dates);
  ...
  const renderLast30Days = () => {
    ...
    return dateRange.map((day) => {
      ...
      const isBrushed = didThatDates.some(date => format(date, 'yyyy-MM-dd') === formattedDay);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-6">
      ...
      >
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">{didThatDates.length} days</h3>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">
                  {didThatDates.length > 0 ? 
                    isToday(didThatDates[0]) ? 'Today' : 
                    isYesterday(didThatDates[0]) ? 'Yesterday' : 
                    format(didThatDates[0], 'MMM d') : 'Never'}
                </h3>
              </div>
        ...
        {/* Recent Diding History */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.8 }}
        >
          <h2 className="text-xl font-semibold text-gray-300 mb-4">Diding History</h2>
          {didThatDates.length > 0 ? (
            <div className="space-y-3">
              <AnimatePresence>
                {didThatDates.slice(0, 10).map((date, index) => (
                  <motion.div
                    key={date.getTime()}
                    initial={{ x: -20, opacity: 0 }}
                    animate={{ x: 0, opacity: 1 }}
            ...
            <div className="text-center py-12 bg-gray-800/30 rounded-xl border border-dashed border-gray-700/50">
              ...
              <h3 className="text-lg font-medium text-gray-400 mb-2">No Diding history yet</h3>
              <p className="text-gray-500 mb-4">Start tracking your Diding habits by adding your first entry</p>
            </div>
          )}
        </motion.div>
      </motion.div>
