...
import { FolderPlus, X, Lock, Globe, Loader2, Star } from 'lucide-react';
import classNames from 'classnames';
import { useAsyncMutation } from '@/hook/mutationHook';
import dayjs from 'dayjs';
import { useUpdateMutation } from '@/redux/api/photo';
...
export default function UpdateFolderDialog({
  ...
  const [updateFolder] = useAsyncMutation(useUpdateMutation);

  useEffect(() => {
    if (folderToUpdate) {
      setName(folderToUpdate.name || '');
      setIsSecure(folderToUpdate.isSecure || false);
    }
  }, [folderToUpdate]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateFolder('Updating folder...', { 
        id: folderToUpdate._id, 
        name, 
        isSecure
      });
      await refetch();
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300">
                          {dayjs(folderToUpdate.createdAt).format('MMM D, YYYY')}
                        </span>
                      </div>
                    </motion.div>
                  )}
