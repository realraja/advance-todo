...
import { Upload, X, Calendar, ChevronDown, ChevronUp, Image, BookOpen, FileText, Star } from 'lucide-react';
import classNames from 'classnames';
import { useAsyncMutation } from '@/hook/mutationHook';
import dayjs from 'dayjs';
import { useUpdateMutation } from '@/redux/api/diary';
...
export default function EditNoteDialog({
  ...
  const [updateNote] = useAsyncMutation(useUpdateMutation);

  useEffect(() => {
    if (showData) {
      setTitle(showData.title || '');
      setContent(showData.content || '');
      setDate(showData.forDate?.split('T')[0] || '');
      setImagePreviews([]);
      setType(showData.type || 'diary');
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 overflow-y-auto scrollEditclass1">
          ...
          >
            <Dialog.Panel
              ...
              exit={{ y: 20, opacity: 0 }}
              className="w-full max-w-md rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 shadow-xl shadow-purple-500/10 border border-gray-700/50 flex flex-col overflow-auto scrollEditclass1"
            ...
            >
              <div className="p-6">
                ...
                <div className="space-y-4 overflow-y-auto scrollEditclass1 flex-1 pr-2">
                  ...
                  >
                    ...
                    <AnimatePresence>
                      {datePickerOpen && (
                        ...
                        >
                          <div className="mt-2 bg-gray-800/70 rounded-lg p-2 border border-gray-700 max-h-48 overflow-y-auto scrollEditclass1">
                  ...
                  >
                    <label className="block text-sm font-medium text-gray-300 mb-1">Add Photos</label>
                    <motion.label
                      whileHover={{ scale: 1.01 }}
                    ...
                    {/* Image Previews - Display only, no delete */}
                    {(imagePreviews.length > 0 || showData?.imgUrl?.length > 0) && (
                      <div className="mt-4 space-y-3">
                        <h4 className="text-xs font-medium text-gray-400 flex items-center gap-2">
                          <Image className="size-4" /> Photos ({imagePreviews.length})
                        </h4>
                        <div className="grid grid-cols-3 gap-2">
                          {showData?.imgUrl?.length >0 && showData?.imgUrl.map((preview, index) => (
                            <motion.div
                              key={index+preview}
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
